...
if __name__ == "__main__":
    NATOMS = 8
    SHOTS_PER_TERM = 50_000
    SEED = 1
    RUN_EXACT_H8_CHECK = False

    print(f"Appendix H reference test: H_{NATOMS}, {SHOTS_PER_TERM} samples / Pauli term")
    ...
    # The actual Appendix-H Monte Carlo algorithm: each H6 braket independently sampled.
    t0 = time.perf_counter()
    E_appendix_h, stderr, E_complex = compute_energy_appendix_h(
        pauli_terms,
        num_orb,
        nelec,
        G,
